<script lang="ts">
  ...
  let selectedQuestionTypes = new Set<QuestionPresentation>(['written']);
  let selectedAnswerModes = new Set<AnswerMode>(['selection']);
  ...
  const QUESTION_TYPES: { value: QuestionPresentation; label: string }[] = [
  ...
  function toggleQuestionType(value: QuestionPresentation) {
    if (selectedQuestionTypes.has(value)) selectedQuestionTypes.delete(value);
    else selectedQuestionTypes.add(value);
    selectedQuestionTypes = selectedQuestionTypes;
  }

  function toggleAnswerMode(value: AnswerMode) {
    if (selectedAnswerModes.has(value)) selectedAnswerModes.delete(value);
    else selectedAnswerModes.add(value);
    selectedAnswerModes = selectedAnswerModes;
  }
  ...
  function startQuiz() {
    ...
    quizEngine.startQuiz(characters, [...selectedQuestionTypes], [...selectedAnswerModes]);
  ...
  function handleResult(correct: boolean) {
    if (correct) {
      ...
      wrongCount++;
      const toastAns = currentQuestion?.character.romaji ?? currentQuestion?.correctAnswer ?? '';
      showToast('wrong', currentQuestion?.character.character ?? '', toastAns, 3000);
    }
    totalAnswered++;
    loadNextQuestion();
  }
  ...
  {#if phase === 'setup'}
    <div class="setup-card">
      ...
      <div class="setup-section">
        ...
        <div class="type-toggles">
          {#each QUESTION_TYPES as { value, label }}
            <button
              class="toggle-btn"
              class:active={selectedQuestionTypes.has(value)}
              on:click={() => toggleQuestionType(value)}>{label}</button
            >
          {/each}
        </div>
      </div>
      ...
      <div class="setup-section">
        ...
        <div class="type-toggles">
          {#each ANSWER_MODES as { value, label }}
            <button
              class="toggle-btn"
              class:active={selectedAnswerModes.has(value)}
              on:click={() => toggleAnswerMode(value)}>{label}</button
            >
          {/each}
        </div>
      </div>
